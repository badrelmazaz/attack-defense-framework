...
const inner_form = await Form.findOne({ where: { id: q.data.id } });
          if (!inner_form || inner_form.owner_id !== req.user.id) {
          continue;
}
  ...
  asyncWrapper(async (req, res) => {
    ...
    try {
      const form = await Form.findByPk(form_id, {
	    ...
	    if (!form || form.owner_id !== req.user.id) {
          console.error(err);
          throw new HttpError(403, 'Gay');
      }

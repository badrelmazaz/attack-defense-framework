...
def login_second():
    ...
    # Check if 'client_token' already exists in the session
    # and if the new token is different from the stored one.
    if 'client_token' in session and session['client_token'] != client_token:
        session.clear() # Invalidate session on token mismatch
        return make_error_response(200, 'Internel Server Error')

    session['client_token'] = client_token
    session['last_sent_nonce'] = server_nonce
...
def login_third():
    ...
    if request.headers.get('User-Agent') != "python-requests/2.31.0":
        session.clear()
        return make_error_response(200, 'Invalid session')
    ...
    last_sent_nonce = session['last_sent_nonce']
    check1 = do_stuff1(client_nonce)
    check2 = do_stuff2(client_nonce)

    if check1 == last_sent_nonce or check2 == last_sent_nonce:
        return make_error_response(200, "Internal Server Error")
